...
class Download(Button):
   ...
   """
    ...
    compo_js_params = Button.compo_js_params + ['download_async']

    # Download the file(s) asynchronous
    ...
    def __init__(self, page, cid, label=None, value=None, event_name=None, event_target=None, is_submit=False,
                 download_async=None, **extra_params):
        """Download component.
        ...
        :param download_async: Download the file(s) asynchronous
        """
        super(Download, self).__init__(page, cid, label, value, event_name, event_target, is_submit, download_async)

    def handle_async_download(self, cid):
        """
        Calls the internal async_download-function which must return the download data and the filename.
        ...
        """
        data, filename = self.async_download()
        if not data:
            return
    ...
    def async_download(self):
        """ Default async_download function

        This is the default async_download function which just returns the download-components value (which typically
